...
if(args$modify_scenario){
  ...
  repeat{
    ...
    cat(blue$bold('3. Parameter space \n'))
    cat(blue$bold('4. Instance set \n'))
    cat(blue$bold('5. Route of options \n'))
    cat(blue$bold('6. Type of scenario \n'))
    ...
    if(opt == 1){
      repeat{
        ...
        cat(magenta('The instance entered already exists, try another name. \n'))
      }
    ...
    if(opt == 3){
      ...
      repeat{
        cat(blue$bold('Enter the new space of parameters for the scenario : \n'))
        newScenarioSpaceParam <- tolower(scan(quiet = T,'stdin', character(), n=1))
        
        checkFile <- paste("./FileSystem/Files/Parameters", newScenarioSpaceParam, sep = "/")
        
        if(file.exists(checkFile)){
          break
        }
        
        cat(magenta('The space of parameters entered does not exists, select an existing one. \n'))
      }
      ...
      oldSpaceParam <- fileData$Parameter.space[fileData$Name == scenarioName]
      fileData$Date.modified[fileData$Name == scenarioName] <- format(Sys.Date(), format="%Y-%m-%d")
      fileData$Parameter.space[fileData$Name == scenarioName] <- newScenarioSpaceParam
      ...
      modificationData <- list("Scenario", 
                               Sys.Date(),
                               scenarioName, 
                               "Modified space parameter", 
                               oldSpaceParam,
                               newScenarioSpaceParam
      )
      write.table(modificationData, file = "./FileSystem/ChangeLog.txt", sep = "," ,row.names = FALSE, col.names = FALSE, append = TRUE)
    }
    ...
    if(opt == 4){
      repeat{
        cat(blue$bold('Enter the new set of instances for the scenario : \n'))
        newScenarioInstanceSet <- tolower(scan(quiet = T,'stdin', character(), n=1))
        
        checkFile <- paste("./FileSystem/Files/Instances", newScenarioInstanceSet, sep = "/")
        
        if(file.exists(checkFile)){
          break
        }
        
        cat(magenta('The set of instances entered does not exists, select an existing one. \n'))
      }
      ...
      oldScenarioInstanceSet <- fileData$Set.of.instances[fileData$Name == scenarioName]
      fileData$Date.modified[fileData$Name == scenarioName] <- format(Sys.Date(), format="%Y-%m-%d")
      fileData$Set.of.instances[fileData$Name == scenarioName] <- newScenarioInstanceSet
      ...
      modificationData <- list("Scenario", 
                               Sys.Date(),
                               scenarioName, 
                               "Modified scenario instance set", 
                               oldScenarioInstanceSet,
                               newScenarioInstanceSet
      )
      write.table(modificationData, file = "./FileSystem/ChangeLog.txt", sep = "," ,row.names = FALSE, col.names = FALSE, append = TRUE)
    }
    if(opt == 5){
      repeat{
        cat(blue$bold('Enter the new route of options : \n'))
        newOptionsRoute <- tolower(scan(quiet = T,'stdin', character(), n=1))
        
        checkFile <- newOptionsRoute
        
        if(file.exists(checkFile)){
          break
        }
        
        cat(magenta('The route of options entered does not exists, write an existing one. \n'))
      }
      
      subDir <- './FileSystem/Scenario.txt'
      fileData <- read.delim(file = subDir, header = TRUE, sep = ",", dec = ".")
      oldOptionsRoute <- fileData$Route.options[fileData$Name == scenarioName]
      
      
      file.remove(oldOptionsRoute)
      file.copy(newOptionsRoute,oldOptionsRoute)
      
      
      
      fileData$Date.modified[fileData$Name == scenarioName] <- format(Sys.Date(), format="%Y-%m-%d")
      fileData$Route.options[fileData$Name == scenarioName] <- newOptionsRoute
      
      newOptionsRoute <- oldOptionsRoute
      #Add data to the file modification
      modificationData <- list("Scenario", 
                               Sys.Date(),
                               scenarioName, 
                               "Modified options route", 
                               oldOptionsRoute,
                               newOptionsRoute
      )
      write.table(modificationData, file = "./FileSystem/ChangeLog.txt", sep = "," ,row.names = FALSE, col.names = FALSE, append = TRUE)
    ...
    if(opt == 6){
      cat(blue$bold('Enter the new type for the scenario: \n'))
      subDir <- './FileSystem/Scenario.txt'
      newScenarioType <- tolower(readLines("stdin", n = 1))
      
      fileData <- read.delim(file = subDir, header = TRUE, sep = ",", dec = ".")
      oldScenarioType <- fileData$Type[fileData$Name == scenarioName]
      
      fileData$Date.modified[fileData$Name == scenarioName] <- format(Sys.Date(), format="%Y-%m-%d")
      fileData$Type[fileData$Name == scenarioName] <- newScenarioType
      #Add data to the file modification
      modificationData <- list("Scenario", 
                               Sys.Date(),
                               scenarioName, 
                               "Modified scenario type", 
                               oldScenarioType,
                               newScenarioType
      )
...
if(args$modify_version){
  repeat{
    cat(blue$bold('Enter the version to modify: '))
    versionName <- tolower(scan(quiet = T,'stdin', character(), n=1))
    
    checkFile <- paste("./FileSystem/Files/Version", versionName, sep = "/")
    
    if(file.exists(checkFile)){
      break
    }
    cat(blue$bold('The entered version does not exist, please try another. \n'))
  }
  ...
  repeat{
    ...
    cat(blue$bold('1. Name \n'))
    cat(blue$bold('2. Description \n'))
    cat(blue$bold('3. Route of version \n'))
    ...
    if(opt == 1){
      repeat{
        cat(blue$bold('Enter the new name for the verion to modify: \n'))
        newVersionName <- scan(quiet = T,'stdin', character(), n=1)
        
        checkFile <- paste("./FileSystem/Files/Version", newVersionName, sep = "/")
        
        if(!file.exists(checkFile)){
          break
        }
        
        cat(magenta("The version entered already exists, try another name. \n"))
      }
      ...
      oldVersionPath <- paste("./FileSystem/Files/Version",oldVersionName,sep = '/')
      newVersionPath <- paste("./FileSystem/Files/Version",newVersionName,sep = '/')
      
      
      
      oldVersionFilePath <- paste(oldVersionPath,oldVersionName,sep = '/')
      newVersionFilePath <- paste(oldVersionPath,newVersionName,sep = '/')
      
      file.rename(oldVersionFilePath,newVersionFilePath)
      file.rename(oldVersionPath,newVersionPath)
      
      subDir <- './FileSystem/Version.txt'
      fileData <- read.delim(file = subDir, header = TRUE, sep = ",", dec = ".")
      fileData$Date.modified[fileData$Version.Number == oldVersionName] <- format(Sys.Date(), format="%Y-%m-%d")
      fileData$Route[fileData$Version.Number == oldVersionName] <- paste(newVersionPath,newVersionName,sep = '/')
      fileData$Version.Number[fileData$Version.Number == oldVersionName] <- newVersionName
      file.remove(subDir)
      write.table(fileData, file = subDir, sep = "," ,row.names = FALSE, col.names = TRUE)
      ...
      fileData$Date.modified[fileData$version == oldVersionName] <- format(Sys.Date(), format="%Y-%m-%d")
      fileData$version[fileData$version == oldVersionName] <- newVersionName
      file.remove(subDir)
      write.table(fileData, file = subDir, sep = ",",row.names = F,col.names = T)
      
      #Add data to the file modification
      modificationData <- list("Version", 
                               Sys.Date(),
                               versionName,
                               "Version name change", 
                               oldVersionName,
